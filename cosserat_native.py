...
class NativeCosseratError(RuntimeError):
    """同梱の羽衣ネイティブランタイムを読み込めない、または進められません。"""
...
def _library_candidates() -> tuple[Path, ...]:
    ...
    explicit = os.environ.get("HAGOROMO_COSSERAT_DLL", "").strip()
    ...
    if sys.platform == "darwin":
        names = ("libhagoromo_cosserat.dylib",)
    elif sys.platform.startswith("win"):
        names = ("hagoromo_cosserat.dll",)
    else:
        names = ("libhagoromo_cosserat.so",)
    search_dirs = (
        root / "bin",
...
def _load_library() -> ctypes.CDLL:
    ...
    for path in _library_candidates():
        ...
        try:
            ...
            raise NativeCosseratError(f"羽衣ネイティブライブラリ {path} を読み込めません: {exc}") from exc
        _configure_library(library)
        version = int(library.hsc_get_api_version())
        if version != API_VERSION:
            raise NativeCosseratError(
                f"ネイティブ羽衣 API {version} が拡張 API {API_VERSION} と一致しません。"
            )
        return library
    raise NativeCosseratError(
        "羽衣ネイティブライブラリが見つかりません。"
        "build_native.ps1 で Windows CUDA ランタイムをビルドしてください。"
...
class NativeCosseratRuntime:
    """ネイティブ布ソルバを 1 つ保持し、羽衣の状態操作を公開する。"""
    ...
    def __init__(
        ...
        *,
        ...
        if self._library.hsc_default_config(ctypes.byref(config)) != 0:
            raise NativeCosseratError("ネイティブソルバが既定設定を提供しませんでした。")
        ...
        if not self._handle:
            raise NativeCosseratError("ネイティブソルバがハンドルを返しませんでした。")
        ...
        if vertex_count.value != self.vertex_count or seam_count.value != self.seam_count:
            self.close()
            raise NativeCosseratError("ネイティブソルバの個数検証に失敗しました。")
    ...
    def _call(self, function_name: str, *arguments) -> None:
        if function_name != "hsc_create" and not self._handle:
            raise NativeCosseratError("羽衣ネイティブランタイムは閉じられています。")
