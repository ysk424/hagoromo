...
HAGOROMO_ROLE = "hagoromo_role"
HAGOROMO_SIMULATION_ROLE = "simulation"
HAGOROMO_PART_ROLE = "simulation_part"
HAGOROMO_BAKED_ROLE = "baked"
HAGOROMO_BAKED_PART_ROLE = "baked_part"


class HagoromoSimulationError(RuntimeError):
...
def _read_vector_attribute(
    ...
    if (
        ...
        raise HagoromoSimulationError(f"{mesh.name} has no valid {name} data.")
    result = np.empty((expected_count, 3), dtype=np.float32)
    attribute.data.foreach_get("vector", result.ravel())
    if not np.all(np.isfinite(result)):
        raise HagoromoSimulationError(f"{mesh.name} contains non-finite {name} data.")
    return result
...
def _source_objects(collection: bpy.types.Collection) -> tuple[bpy.types.Object, ...]:
    if collection is None or collection.get(YOHSAI_ROLE) != YOHSAI_CLOTHES_ROLE:
        raise HagoromoSimulationError("Yohsai の服コレクションを選択してください。")
    names = [str(value) for value in collection.get(YOHSAI_PART_NAMES, [])]
    if len(names) < 2 or len(set(names)) != len(names):
        raise HagoromoSimulationError(
            "保存済み Yohsai Gravity 状態に一意のパーツ順序がありません。"
        )
    by_name = {
        obj.name: obj
    ...
    if any(obj.get(YOHSAI_GRAVITY_STATE) != "DONE" for obj in parts):
        raise HagoromoSimulationError("すべての Yohsai パーツに Gravity 完了状態が必要です。")
    if int(collection.get(YOHSAI_REVISION, 0)) <= 0:
        raise HagoromoSimulationError("Yohsai コレクションに Gravity 完了リビジョンがありません。")
    backend = str(collection.get(YOHSAI_BACKEND, "STABLE_COSSERAT"))
    if backend != "STABLE_COSSERAT":
        raise HagoromoSimulationError(f"The saved Yohsai solver {backend!r} is not supported.")
    return parts
...
def _cloth_topology(parts: Iterable[PartRange]) -> ClothTopology:
    ...
    for part in parts:
        ...
        if (
            ...
            raise HagoromoSimulationError(f"{part.source.name} has no valid pattern coordinates.")
        if (
            ...
            raise HagoromoSimulationError(f"{part.source.name} has no valid material edge lengths.")
        if (
            ...
            raise HagoromoSimulationError(f"{part.source.name} has no valid grainline edge map.")
        if (
            ...
            raise HagoromoSimulationError(f"{part.source.name} has no valid grainline quad map.")
        ...
        if not np.all(np.isfinite(pattern)) or not np.all(np.isfinite(local_rest)):
            raise HagoromoSimulationError(f"{part.source.name} contains non-finite material data.")
        ...
        for edge in mesh.edges:
            ...
            if not rest_length > 1.0e-8:
                raise HagoromoSimulationError(f"{part.source.name} has a zero-length material edge.")
...
def read_source_state(collection: bpy.types.Collection) -> SourceState:
    ...
    for obj in objects:
        ...
        if count <= 0:
            raise HagoromoSimulationError(f"{obj.name} has no cloth vertices.")
        positions = _world_vertices(obj)
        if not np.all(np.isfinite(positions)):
            raise HagoromoSimulationError(f"{obj.name} contains non-finite world positions.")
        ...
        if len(stored_matrix) != 16:
            raise HagoromoSimulationError(f"{obj.name} has no saved Yohsai transform.")
        if not np.allclose(
            stored_matrix,
    ...
    if not seam_values.size or seam_values.ndim != 1 or seam_values.size % 2:
        raise HagoromoSimulationError("保存済み Yohsai 縫い合わせペアが欠落しているか不正です。")
    ...
    if (
        ...
        raise HagoromoSimulationError("保存済み Yohsai 縫い合わせペアが服頂点と一致しません。")
    ...
    if seam_state.shape != (len(seams),) or not np.all(np.isfinite(seam_state)):
        raise HagoromoSimulationError("保存済み Yohsai Seam 状態が縫い合わせペアと一致しません。")
...
def body_snapshot(
    ...
    *,
    ...
    if body is None or body.type != "MESH":
        raise HagoromoSimulationError("アーマチュア変形のボディメッシュを選択してください。")
    ...
    if not len(vertices) or not len(faces):
        raise HagoromoSimulationError("評価済みボディに衝突三角形がありません。")
    if not np.all(np.isfinite(vertices)):
        raise HagoromoSimulationError("評価済みボディに非有限頂点があります。")
    bvh = None
    if build_cpu_bvh:
...
def _ray_intersection_count(body: BodySnapshot, point: Vector, direction: Vector) -> int:
    if body.bvh is None:
        raise HagoromoSimulationError("要求された CPU ボディ BVH が構築されていません。")
    count = 0
    origin = point.copy()
...
def required_body_substeps(maximum_movement_m: float, limit_m: float) -> int:
    if not math.isfinite(limit_m) or limit_m <= 0.0:
        raise HagoromoSimulationError("最大ボディステップは 0 より大きい必要があります。")
    if not math.isfinite(maximum_movement_m) or maximum_movement_m < 0.0:
        raise HagoromoSimulationError("ボディ移動量が無効です。")
    count = max(1, int(math.ceil(maximum_movement_m / limit_m - 1.0e-9)))
    if count > MAX_BODY_SUBSTEPS:
        raise HagoromoSimulationError(
            f"1 フレームに {count} 回のボディステップが必要です。安全上限は {MAX_BODY_SUBSTEPS} です。"
        )
    return count
...
def remove_previous_outputs(source_collection: bpy.types.Collection) -> bool:
    ...
    for collection in list(bpy.data.collections):
        if (
            collection.get(HAGOROMO_ROLE) == HAGOROMO_SIMULATION_ROLE
            and collection.get("hagoromo_source_collection") == source_collection.name
        ):
...
def create_output_parts(
    ...
    collection = bpy.data.collections.new(f"{source_collection.name}_HAGOROMO")
    scene.collection.children.link(collection)
    collection[HAGOROMO_ROLE] = HAGOROMO_SIMULATION_ROLE
    collection["hagoromo_source_collection"] = source_collection.name
    collection["hagoromo_start_frame"] = int(start_frame)
    collection["hagoromo_end_frame"] = int(end_frame)
    collection["hagoromo_maximum_body_step_cm"] = float(maximum_step_cm)
    collection["hagoromo_contact_clearance_cm"] = float(contact_clearance_cm)
    collection["hagoromo_solver_iterations"] = int(solver_iterations)
    collection["hagoromo_internal_substeps"] = INTERNAL_SUBSTEPS
    collection["hagoromo_backend"] = "CUDA_RESIDENT"
    ...
    try:
        for part in source_parts:
            ...
            output.name = f"{source.name}_HAGOROMO"
            output.data.name = f"{source.data.name}_HAGOROMO"
            ...
            for key in list(output.keys()):
                del output[key]
            output[HAGOROMO_ROLE] = HAGOROMO_PART_ROLE
            output["hagoromo_source_object"] = source.name
            output["hagoromo_part_index"] = len(output_parts)
...
def _set_eval_time_interpolation_linear(keys: bpy.types.Key) -> None:
    ...
    if action is None:
        raise HagoromoSimulationError(f"{keys.name} がベイク用 Action を作成しませんでした。")
    ...
    if not eval_fcurves:
        raise HagoromoSimulationError(f"{keys.name} が eval_time の F-Curve を作成しませんでした。")
...
def bake_output_collection(collection: bpy.types.Collection) -> str:
    ...
    if (
        collection is None
        or collection.get(HAGOROMO_ROLE) != HAGOROMO_SIMULATION_ROLE
        or not bool(collection.get("hagoromo_cache_ready", False))
    ):
        raise HagoromoSimulationError("ベイクできる完成済みの羽衣シミュレーションがありません。")
    parts = tuple(
        obj
        for obj in collection.objects
        if obj.type == "MESH" and obj.get(HAGOROMO_ROLE) == HAGOROMO_PART_ROLE
    )
    if not parts:
        raise HagoromoSimulationError("完成済みの羽衣シミュレーションに出力パーツがありません。")
    start_frame = int(collection.get("hagoromo_start_frame", 0))
    end_frame = int(collection.get("hagoromo_end_frame", 0))
    if end_frame <= start_frame:
        raise HagoromoSimulationError("完成済みの羽衣シミュレーションのフレーム範囲が無効です。")
    ...
    for obj in parts:
        ...
        if keys is None or len(keys.key_blocks) < end_frame - start_frame + 2:
            raise HagoromoSimulationError(f"{obj.name} に完成した羽衣 Shape Key キャッシュがありません。")
        frame_values: list[tuple[int, float]] = []
        for frame in range(start_frame, end_frame + 1):
            shape = keys.key_blocks.get(f"HAGOROMO_{frame:04d}")
            if shape is None:
                raise HagoromoSimulationError(
                    f"{obj.name} にフレーム {frame} の羽衣 Shape Key がありません。"
                )
    ...
    for _obj, keys, frame_values in bake_data:
        ...
        for frame, value in frame_values:
            ...
            if not keys.keyframe_insert(data_path="eval_time", frame=frame, group="羽衣ベイク"):
                raise HagoromoSimulationError(
                    f"Could not keyframe {keys.name} eval_time at frame {frame}."
                )
        _set_eval_time_interpolation_linear(keys)

    source_name = str(collection.get("hagoromo_source_collection", "HAGOROMO"))
    collection[HAGOROMO_ROLE] = HAGOROMO_BAKED_ROLE
    collection["hagoromo_baked"] = True
    collection.name = f"{source_name}_HAGOROMO_BAKED"
    for obj, _keys, _frame_values in bake_data:
        source_object = str(obj.get("hagoromo_source_object", obj.name))
        obj[HAGOROMO_ROLE] = HAGOROMO_BAKED_PART_ROLE
        obj["hagoromo_baked"] = True
        obj.name = f"{source_object}_HAGOROMO_BAKED"
        obj.data.name = f"{source_object}_HAGOROMO_BAKED_MESH"
...
def _install_shape_key_cache(
    ...
    if not frames:
        raise HagoromoSimulationError("シミュレーションがアニメーションフレームを生成しませんでした。")
    for part in parts:
        output = part.output
        if output is None:
            continue
        ...
        for frame in frames:
            shape = output.shape_key_add(name=f"HAGOROMO_{frame:04d}", from_mix=False)
        ...
        if len(key_frames) < 2:
            raise HagoromoSimulationError("キャッシュには少なくとも 2 フレームが必要です。")
        if any(right <= left for left, right in zip(key_frames, key_frames[1:])):
            raise HagoromoSimulationError("Blender が予期しない絶対 Shape Key スケールを割り当てました。")
        ...
        for frame, key_frame in zip(frames, key_frames):
            ...
            point.interpolation = "LINEAR"
        output["hagoromo_cache_start"] = int(frames[0])
        output["hagoromo_cache_end"] = int(frames[-1])
...
class SimulationRunner:
    ...
    def __init__(
        ...
        if end_frame <= start_frame:
            raise HagoromoSimulationError("終了フレームは開始フレームより大きい必要があります。")
        ...
        if (
            ...
            raise HagoromoSimulationError(
                f"接触クリアランスは 0 より大きく、最大 "
                f"{COLLISION_SEARCH_M * 100.0:g} cm 以下である必要があります。"
            )
        if not 1 <= int(solver_iterations) <= 128:
            raise HagoromoSimulationError("ソルバ反復回数は 1 から 128 の範囲である必要があります。")
    ...
    def status(self) -> str:
        ...
        if self.pending_steps:
            ...
            return (
                f"フレーム {self.current_frame}->{self.current_frame + 1}: "
                f"ボディステップ {completed}/{self.last_interval_substeps}"
            )
        return f"フレーム準備中 {self.current_frame}->{self.current_frame + 1}"
    ...
    def _prepare_interval(self, context) -> None:
        ...
        while True:
            ...
            if count > MAX_BODY_SUBSTEPS:
                raise HagoromoSimulationError(
                    f"フレーム {self.current_frame} は "
                    f"{MAX_BODY_SUBSTEPS} を超えるボディステップを要求しています。"
                )
    ...
    def advance_one(self, context) -> bool:
        ...
        if self.runtime is None:
            raise HagoromoSimulationError("ネイティブシミュレーションランタイムは閉じられています。")
        try:
            ...
            raise HagoromoSimulationError(str(exc)) from exc
        ...
        if not self.pending_steps:
            ...
            if not np.all(np.isfinite(positions)) or not np.all(np.isfinite(velocities)):
                self.runtime.replace_state(self.positions, self.velocities, self.locked)
                raise HagoromoSimulationError("ソルバが非有限な服状態を生成しました。")
    ...
    def finish(self) -> str:
        if self.current_frame < self.end_frame or self.pending_steps:
            raise HagoromoSimulationError("シミュレーションが終了フレームに達していません。")
        _install_shape_key_cache(self.output_parts, self.frame_cache)
        if self.output_collection is not None:
            self.output_collection["hagoromo_cache_ready"] = True
            self.output_collection["hagoromo_maximum_substeps"] = int(self.maximum_substeps)
            self.output_collection["hagoromo_maximum_body_movement_cm"] = float(
                self.maximum_observed_body_movement_m * 100.0
            )
            self.output_collection["hagoromo_maximum_body_vertex"] = int(
                self.maximum_movement_vertex
            )
            self.output_collection["hagoromo_maximum_contact_passes_per_frame"] = int(
                self.maximum_substeps * INTERNAL_SUBSTEPS * self.solver_iterations
            )
            self.output_collection["hagoromo_body_object"] = self.body_object.name
        if self.runtime is not None:
            self.runtime.close()
            self.runtime = None
        ...
        self.finished = True
        return (
            f"CUDA キャッシュ {self.start_frame}-{self.end_frame}; "
            f"最大ボディ移動 {self.maximum_observed_body_movement_m * 100.0:.3f} cm; "
            f"最大 {self.maximum_substeps} 回の重力呼び出し / "
            f"フレームあたり最大 {self.maximum_substeps * INTERNAL_SUBSTEPS * self.solver_iterations} "
            f"接触パス"
        )
