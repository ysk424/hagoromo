...
requested_collection = os.environ.get("HAGOROMO_TEST_CLOTHES", "").strip()
collection = (
...
assert bool(runner.output_collection["hagoromo_cache_ready"])
assert runner.output_collection["hagoromo_backend"] == "CUDA_RESIDENT"
assert len(runner.output_parts) == len(source_state.parts)
assert int(runner.output_collection["hagoromo_maximum_substeps"]) == 6
assert float(runner.output_collection["hagoromo_contact_clearance_cm"]) == 1.0
assert int(runner.output_collection["hagoromo_solver_iterations"]) == 20
assert int(runner.output_collection["hagoromo_maximum_contact_passes_per_frame"]) == 960
assert all(part.output.data.shape_keys is not None for part in runner.output_parts)
print("HAGOROMO_SAVED_SIMULATION_OK", summary)
